...
/proc/power_restore(var/announce = 1)
	...
	if(announce)
		command_announcement.Announce("Power has been restored to the colony. We apologize for the inconvenience.", "Power Systems Nominal", new_sound = 'sound/AI/poweron.ogg')
	for(var/obj/machinery/power/apc/C in GLOB.apc_list)
		C.failure_timer = 0
		if(C.cell)
...
/proc/power_restore_quick(var/announce = 1)
	...
	if(announce)
		command_announcement.Announce("All SMESs on within the colony have been recharged from reserve grids. We apologize for the inconvenience.", "Power Systems Nominal", new_sound = 'sound/AI/poweron.ogg')
	for(var/obj/machinery/power/smes/S in GLOB.smes_list)
		S.failure_timer = 0
		S.charge = S.capacity
